...
public static class Utilities
{
    ...
    public static void DebugKTex(RenderTexture rt, int N)
    {
        // Backup currently active RT
        RenderTexture prev = RenderTexture.active;

        // Make a readable texture
        Texture2D tex2D = new Texture2D(N, N, TextureFormat.RGFloat, false, true);

        // Copy GPU -> CPU
        RenderTexture.active = rt;
        tex2D.ReadPixels(new Rect(0, 0, N, N), 0, 0);
        tex2D.Apply();

        // Restore active RT
        RenderTexture.active = prev;

        // Print a few sample texels
        // Debug.Log("Center (N/2,N/2): " + tex2D.GetPixel(N / 2, N / 2));
        // Debug.Log("Corner (0,0): " + tex2D.GetPixel(0, 0));
        // Debug.Log("Right edge (N-1,N/2): " + tex2D.GetPixel(N - 1, N / 2));
        // Debug.Log("Top edge (N/2,N-1): " + tex2D.GetPixel(N / 2, N - 1));

        // Or iterate if you want to dump them all
        for (int y = 0; y < N; y++)
        {
            for (int x = 0; x < N; x++)
            {
                Color c = tex2D.GetPixel(x, y);
                Debug.Log($"({x},{y}) : {c.r}, {c.g}");
            }
        }
    }
}
